...
def showGenshapeColor(genshape, color):
    recover_boxes = genshape
...
def outputObbs(genshape, fileName):
    recover_boxes = genshape
    boxNum = len(recover_boxes)

    fo = open(fileName, "w")
    fo.write("#GRASS2 Obbs outputs:\n")
    fo.write("N %s\n"%(str(boxNum)))
    for jj in range(len(recover_boxes)):
        tmpPoint = recover_boxes[jj][:]
        center = tmpPoint[0: 3]
        lengths = tmpPoint[3: 6]
        dir_1 = tmpPoint[6: 9]
        dir_2 = tmpPoint[9: ]
        ...
        fo.write("%s %s %s "%(str(center[0]), str(center[1]), str(center[2])))
        fo.write("%s %s %s "%(str(dir_1[0]), str(dir_1[1]), str(dir_1[2])))
        fo.write("%s %s %s "%(str(dir_2[0]), str(dir_2[1]), str(dir_2[2])))
        fo.write("%s %s %s "%(str(dir_3[0]), str(dir_3[1]), str(dir_3[2])))
        fo.write("%s %s %s\n"%(str(lengths[0]), str(lengths[1]), str(lengths[2])))
    fo.close()
